...
public static class DataSeeder
{
    public static async Task SeedAsync(IServiceProvider serviceProvider)
    {
        ...
        if (!await context.Contracts.AnyAsync())
        {
            ...
            var term1 = new ContractTerm
            {
                Id = Guid.NewGuid(),
                ContractId = contract.Id,
                Title = "Liability",
                Content = "The contractor shall be liable for all damages up to $1,000,000."
            };

            var term2 = new ContractTerm
            {
                Id = Guid.NewGuid(),
                ContractId = contract.Id,
                Title = "Payment Terms",
                Content = "Net 30 days after invoice is received and approved by engineering."
            };
            ...
            var drawing = new DrawingAttachment
            {
                Id = Guid.NewGuid(),
                ContractId = contract.Id,
                FileName = "Foundation_Plan_RevA.pdf",
                FileUrl = "/mock/Foundation_Plan_RevA.pdf",
                UploadedAt = DateTimeOffset.UtcNow
            };

            context.Contracts.Add(contract);
            context.ContractItems.AddRange(item1, item2);
            context.ContractTerms.AddRange(term1, term2);
            context.DrawingAttachments.Add(drawing);
